...
impl MainWindow {
                    ...
                    WindowEvent::Resized(window_w, window_h) => {
                        if let Some(ref image) = self.image_texture {
                            ...
                            if img_ratio < window_ratio {
                                self.scale.x = ((img_ratio / window_ratio) * window_w as f32)
                                    .floor()
                                    / window_w as f32;
                            } else {
                                self.scale.y = ((window_ratio / img_ratio) * window_h as f32)
                                    .floor()
                                    / window_h as f32;
                            }
                        }
    ...
    fn draw(&self) {
        ...
        if let Some(ref texture) = self.image_texture {
            let transform = Matrix4::from_nonuniform_scale(self.scale.x, self.scale.y, 1.0);
            let transform = Matrix4::from_translation(-1.0 * self.cam_pos.extend(0.0)) * transform;
            let transform = Matrix4::from_scale(self.zoom_scale) * transform;
