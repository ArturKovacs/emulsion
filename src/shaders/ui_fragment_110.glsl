...
void main() {
    ...
    color = vec4(mix(color.rgb, vec3(1.0), max(0.0, brighten*color.a)), color.a);
    color = vec4(mix(color.rgb, vec3(0.0), -min(0.0, brighten*color.a)), color.a);
    ...
    gl_FragColor = mix(color, vec4(vec3(0.0), 1.0), shadow);
}
